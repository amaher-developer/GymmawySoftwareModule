    ...
    @if(count($records) > 0)
        <table class="table card-table table-striped table-vcenter text-nowrap mb-0  " >
            ...
            <tr style="float: right">
                @foreach($keys as $key)
                    <th>{{trans('sw.notes')}}</th>
                    <th>{{trans('sw.plan_diet')}}</th>
                    <th>{{trans('sw.plan_training')}}</th>
                    <th>{{trans('sw.diseases')}}</th>
                    <th>{{trans('sw.weight')}}</th>
                    <th>{{trans('sw.height')}}</th>
                    <th>{{trans('sw.name')}}</th>
                    <th>{{trans('sw.barcode')}}</th>
                @endforeach
            </tr>
            </thead>
            <tbody>
            ...
            @foreach($records as $record)
                <tr>
                        <td> {{ $record->notes }}</td>
                        <td> {{ $record->diet_plan_details }}</td>
                        <td> {{ $record->training_plan_details }}</td>
                        <td> {{ $record->diseases }}</td>
                        <td> {{ $record->weight }}</td>
                        <td> {{ $record->height }}</td>
                        <td> {{ $record->member->name }}</td>
                        <td> {{ $record->member->code }}</td>
