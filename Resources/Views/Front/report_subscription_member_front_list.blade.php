    ...
    <div class="card-header align-items-center py-5 gap-2 gap-md-5">
        ...
        <div class="card-toolbar">
            <div class="d-flex flex-wrap align-items-center gap-2 gap-lg-3">
                <!--begin::Filter-->
                <button type="button" class="btn btn-sm btn-flex btn-light-primary" data-bs-toggle="collapse" data-bs-target="#kt_subscription_members_filter_collapse">
                ...
                <!--begin::Export-->
                @if((count(array_intersect(@(array)$swUser->permissions, ['exportSubscriptionMemberPDF', 'exportSubscriptionMemberExcel'])) > 0) || $swUser->is_super_user)
                    <div class="m-0">
                        ...
                        <div class="menu menu-sub menu-sub-dropdown w-200px" data-kt-menu="true">
                            @if(in_array('exportSubscriptionMemberExcel', (array)$swUser->permissions) || $swUser->is_super_user)
                                <div class="menu-item px-3">
                                    <a href="{{route('sw.exportSubscriptionMemberExcel', request()->query())}}" class="menu-link px-3">
                                        <i class="ki-outline ki-file-down fs-6 me-2"></i>
                                        {{ trans('sw.excel_export') }}
                                    </a>
                                </div>
                            @endif
                            @if(in_array('exportSubscriptionMemberPDF', (array)$swUser->permissions) || $swUser->is_super_user)
                                <div class="menu-item px-3">
                                    <a href="{{route('sw.exportSubscriptionMemberPDF', request()->query())}}" class="menu-link px-3">
                                        <i class="ki-outline ki-file-down fs-6 me-2"></i>
                                        {{ trans('sw.pdf_export') }}
                                    </a>
                                </div>
    ...
    <div class="card-body pt-0">
        ...
        @if(count($logs) > 0)
            ...
            <div class="table-responsive">
                <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_subscription_members_table">
                    <thead>
                            ...
                            <tr>
                                ...
                                <td class="text-end actions-column">
                                    <div class="d-flex justify-content-end align-items-center gap-1 flex-wrap">
                                        @if((in_array('createMemberPayAmountRemainingForm', (array)$swUser->permissions) || $swUser->is_super_user) && @$log->member)
                                            <!--begin::Pay-->
                                        ...
                                        @if(@$log->id)
                                            <!--begin::Invoice-->
                                            <a href="{{route('sw.showOrderSubscription',@$log->id)}}" style="cursor: pointer;"
                                        ...
                                        <table class="table align-middle table-row-dashed fs-6 gy-5">
                                            <thead>
                                                <tr class="text-gray-500 fw-bold fs-7 text-uppercase gs-0">
                                                    ...
                                                    <th class="min-w-100px">{{ trans('sw.amount_remaining')}}</th>
                                                    <th class="min-w-70px text-end">{{ trans('admin.actions')}}</th>
                                                </tr>
                                            </thead>
                                            <tbody class="fw-semibold text-gray-600">
                                                @php $remain_amount_subscriptions_check = false @endphp
                                                @if(@count($log->member->member_remain_amount_subscriptions) > 0)
                                                    @php $remain_amount_subscriptions_check = true @endphp
                                                    @foreach($log->member->member_remain_amount_subscriptions as $member_remain_amount)
                                                    <tr id="tr_pay_{{$member_remain_amount->id}}">
                                                        <td>{{@$member_remain_amount->subscription->name}}</td>
