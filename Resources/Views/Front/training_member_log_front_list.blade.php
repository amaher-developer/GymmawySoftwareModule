    ...
    <div class="card-body pt-0">
        ...
        <div class="d-flex align-items-center mb-5">
            ...
            <div class="d-flex flex-column">
                ...
                <span class="fs-2 fw-bold text-primary">{{ count($logs) }}</span>
            </div>
        </div>
        <!--end::Total count-->
        ...
        @if(count($logs) > 0)
            ...
            <div class="table-responsive">
                <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_training_member_logs_table">
                <thead>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="d-flex align-items-center">
                                <div class="symbol symbol-circle symbol-50px overflow-hidden me-3">
                                    @if($log->member->image)
                                        <div class="symbol-label">
                                            <img src="{{asset('uploads/members/'.$log->member->image)}}" alt="{{$log->member->name}}" class="w-100">
                                        </div>
                                    @else
                                ...
                                <div class="d-flex flex-column">
                                    <span class="text-gray-800 text-hover-primary mb-1">{{ $log->member->name }}</span>
                                    <span class="text-muted fs-7">{{ $log->member->phone }}</span>
                                </div>
                            </div>
                        </td>
                        <td>
                            <div class="d-flex align-items-center">
                                ...
                                <div class="d-flex flex-column">
                                    <span class="text-gray-800 text-hover-primary mb-1">{{ $log->training->name ?? '' }}</span>
                                    @if($log->training->description)
                                        <span class="text-muted fs-7">{{ Str::limit($log->training->description, 30) }}</span>
                                    @endif
                                </div>
                            </div>
                        </td>
                        <td>
                            ...
                            @else
                                <span class="badge badge-light-secondary">{{ trans('sw.no_action')}}</span>
                            @endif
                        </td>
                        ...
                        <td class="text-end actions-column">
                            <div class="d-flex justify-content-end align-items-center gap-1 flex-wrap">
                                @if(in_array('showTrainingMemberLog', (array)$swUser->permissions) || $swUser->is_super_user)
                                    <a href="{{route('sw.showTrainingMemberLog',$log->id)}}" class="btn btn-sm btn-light btn-active-light-primary">
                                        <i class="ki-outline ki-eye fs-6"></i>
                                    </a>
                                @endif
                                @if(in_array('deleteTrainingMemberLog', (array)$swUser->permissions) || $swUser->is_super_user)
                                    <a href="{{route('sw.deleteTrainingMemberLog',$log->id)}}" class="confirm_delete btn btn-sm btn-light-danger">
            ...
            <div class="text-center py-10">
                <div class="symbol symbol-100px mb-5">
                    <div class="symbol-label fs-2x fw-semibold text-success bg-light-success">
                        <i class="ki-outline ki-notification fs-2x text-success"></i>
                    </div>
                </div>
