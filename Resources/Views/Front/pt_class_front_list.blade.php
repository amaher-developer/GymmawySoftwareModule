    ...
    <div class="card-header align-items-center py-5 gap-2 gap-md-5">
        ...
        <div class="card-toolbar">
            <div class="d-flex align-items-center gap-2 gap-lg-3">
                <!--begin::Add PT Class-->
                @if(in_array('createPTClass', (array)$swUser->permissions) || $swUser->is_super_user)
                    <a href="{{route('sw.createPTClass')}}" class="btn btn-sm btn-flex btn-light-primary">
    ...
    <div class="card-body pt-0">
        ...
        @if(count($classes) > 0)
            ...
            <div class="table-responsive">
                <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_pt_classes_table">
                <thead>
                    <tr class="text-gray-500 fw-bold fs-7 text-uppercase gs-0">
                        <th class="min-w-200px">
                            <i class="ki-outline ki-notepad fs-6 me-2"></i>{{ trans('sw.name')}}
                        </th>
                        <th class="min-w-200px">
                            <i class="ki-outline ki-notepad fs-6 me-2"></i>{{ trans('sw.pt_subscription')}}
                        </th>
                        <th class="min-w-100px">
                            <i class="ki-outline ki-sort fs-6 me-2"></i>{{ trans('sw.pt_classes_num')}}
                        </th>
                        <th class="min-w-100px">
                ...
                <tbody class="fw-semibold text-gray-600">
                    @foreach($classes as $key=> $class)
                        <tr>
                            ...
                            <td class="pe-0">
                                <span class="fw-bold">{{ $class->pt_subscription->name }}</span>
                            </td>
                            <td class="pe-0">
                                <span class="fw-bold text-primary">{{ $class->classes }}</span>
                            </td>
                            <td class="pe-0">
                            ...
                            <td class="text-end actions-column">
                                <div class="d-flex justify-content-end align-items-center gap-1 flex-wrap">
                                    @if(in_array('editPTClass', (array)$swUser->permissions) || $swUser->is_super_user)
                                        <!--begin::Edit-->
                                        <a href="{{route('sw.editPTClass',$class->id)}}"
                                    ...
                                    @if(in_array('deletePTClass', (array)$swUser->permissions) || $swUser->is_super_user)
                                        @if(request('trashed'))
                                            <!--begin::Enable-->
                                            <a title="{{ trans('admin.enable')}}"
                                               href="{{route('sw.deletePTClass',$class->id)}}"
            ...
            <div class="d-flex flex-stack flex-wrap pt-10">
                <div class="fs-6 fw-semibold text-gray-700">
                    {{ trans('sw.showing_entries', [
                        'from' => $classes->firstItem() ?? 0,
                        'to' => $classes->lastItem() ?? 0,
                        'total' => $classes->total()
                    ]) }}
                </div>
                <ul class="pagination">
                    {!! $classes->appends($search_query)->render() !!}
                </ul>
            </div>
            <!--end::Pagination-->
        @else
