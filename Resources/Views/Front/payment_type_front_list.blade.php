    ...
    <div class="card-header align-items-center py-5 gap-2 gap-md-5">
        ...
        <div class="card-toolbar">
            <div class="d-flex align-items-center gap-2 gap-lg-3">
                ...
                @if(in_array('createPaymentType', (array)$swUser->permissions) || $swUser->is_super_user)
                    <a href="{{route('sw.createPaymentType')}}" class="btn btn-sm btn-flex btn-light-primary">
    ...
    <div class="card-body pt-0">
        ...
        <div class="d-flex align-items-center mb-5">
            ...
            <div class="d-flex flex-column">
                ...
                <span class="fs-2 fw-bold text-primary">{{ count($payment_types) }}</span>
            </div>
        </div>
        <!--end::Total count-->
        ...
        @if(count($payment_types) > 0)
            ...
            <div class="table-responsive">
                <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_payment_types_table">
                <thead>
                        ...
                        <tr>
                            ...
                            <td class="text-end actions-column">
                                <div class="d-flex justify-content-end align-items-center gap-1 flex-wrap">
                                    @if(in_array('editPaymentType', (array)$swUser->permissions) || $swUser->is_super_user)
                                        <!--begin::Edit-->
                                        <a href="{{route('sw.editPaymentType',$payment_type->id)}}"
                                    ...
                                    @if(in_array('deletePaymentType', (array)$swUser->permissions) || $swUser->is_super_user)
                                        @if(request('trashed'))
                                            <!--begin::Enable-->
                                            <a title="{{ trans('admin.enable')}}"
                                               href="{{route('sw.deletePaymentType',$payment_type->id)}}"
            ...
            <div class="d-flex flex-stack flex-wrap pt-10">
                <div class="fs-6 fw-semibold text-gray-700">
                    Showing {{ $payment_types->firstItem() ?? 0 }} to {{ $payment_types->lastItem() ?? 0 }} of {{ $payment_types->total() }} entries
                </div>
                <ul class="pagination">
                    {!! $payment_types->appends($search_query)->render() !!}
                </ul>
            </div>
            <!--end::Pagination-->
        @else
