...
<table class="items">
    ...
    <tbody>
    <tr>
        <td>{{ trans('sw.member_moneybox_add_msg', ['member'=> $order->member->name, 'subscription' => $order->subscription->name, 'amount_paid' => $order->amount_paid, 'amount_remaining' => $order->amount_remaining])}}
            @if($order->discount_value) {{ trans('sw.discount_msg', ['value' => $order->discount_value])}} @endif
        </td>
        <td>1</td>
        <td class="" style="text-align: center">{{number_format($order->amount_paid, 2)}} {{@trans('sw.app_currency')}}</td>
    </tr>

    @if(@$order->vat)
    <tr>
        <th colspan="2" class="total text">{{ trans('sw.total_for_price')}} ({{ trans('sw.excluding_vat')}})</th>
        <th class="total ">{{number_format(($order->amount_paid + @$order->amount_remaining - @$order->vat),2)}} {{@trans('sw.app_currency')}}</th>
    </tr>
    <tr>
        <th colspan="2" class="total text">{{ trans('sw.total_for_price')}} ({{ trans('sw.including_vat')}})</th>
        <th class="total ">{{number_format(($order->amount_paid + @$order->amount_remaining),2)}} {{@trans('sw.app_currency')}}</th>
    </tr>
    <tr>
        <th colspan="2" class="total text">{{ trans('sw.discount')}}</th>
        <th class="total ">{{number_format($order->discount_value, 2)}} {{@trans('sw.app_currency')}}</th>
    </tr>
    ...
    @else
        <tr>
            <th colspan="2" class="total text">{{ trans('sw.total_price')}}</th>
            <th class="total ">{{number_format(($order->amount_paid + @$order->amount_remaining),2)}} {{@trans('sw.app_currency')}}</th>
        </tr>
        <tr>
            <th colspan="2" class="total text">{{ trans('sw.discount')}}</th>
    ...
    @endif
    <tr>
        <th colspan="2" class="total text">{{ trans('sw.reminder')}}</th>
        <th class="total ">{{@number_format($order->amount_remaining, 2)}} {{@trans('sw.app_currency')}}</th>
    </tr>

    <tr>
        <th colspan="2" class="total text">{{ trans('sw.paid')}}</th>
        <th class="total ">{{@number_format($order->amount_paid, 2)}} {{@trans('sw.app_currency')}}</th>
    </tr>

{{--    <tr>--}}
{{--        <th colspan="2" class="total text">Total Gross</th>--}}
{{--        <th class="total ">{{number_format(($order->amount_paid - @$vat), 2)}} {{@trans('sw.app_currency')}}</th>--}}
{{--    </tr>--}}

{{--    <tr>--}}
{{--        <th colspan="2" class="total text">VAT ({{@$mainSettings->vat_details['vat_percentage'].'%'}})</th>--}}
{{--        <th class="total ">{{@number_format($vat, 2)}} {{@trans('sw.app_currency')}}</th>--}}
{{--    </tr>--}}
