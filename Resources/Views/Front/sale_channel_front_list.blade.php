...
<div class="card card-flush">
    ...
    <div class="card-header align-items-center py-5 gap-2 gap-md-5">
        ...
        <div class="card-toolbar">
            <div class="d-flex align-items-center gap-2 gap-lg-3">
                <!--begin::Add Sale Channel-->
                @if(in_array('createSaleChannel', (array)$swUser->permissions) || $swUser->is_super_user)
                    <a href="{{route('sw.createSaleChannel')}}" class="btn btn-sm btn-flex btn-light-primary">
                        <i class="ki-outline ki-plus fs-6"></i>
                        {{ trans('admin.add')}}
                    </a>
                @endif
                <!--end::Add Sale Channel-->
            </div>
        </div>
    </div>
    <!--end::Card header-->
    ...
    <div class="card-body pt-0">
        ...
        @if(count($sale_channels) > 0)
            <!--begin::Table-->
            <div class="table-responsive">
                <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_sale_channels_table">
                <thead>
                ...
                <tbody class="fw-semibold text-gray-600">
                    @foreach($sale_channels as $key=> $sale_channel)
                        <tr>
                            <td>
                            ...
                            <td class="text-end actions-column">
                                <div class="d-flex justify-content-end align-items-center gap-1 flex-wrap">
                                    @if(in_array('editSaleChannel', (array)$swUser->permissions) || $swUser->is_super_user)
                                        <!--begin::Edit-->
                                        <a href="{{route('sw.editSaleChannel',$sale_channel->id)}}"
                                    ...
                                    @if(in_array('deleteSaleChannel', (array)$swUser->permissions) || $swUser->is_super_user)
                                        @if(request('trashed'))
                                            <!--begin::Enable-->
                                            <a title="{{ trans('admin.enable')}}"
                                               href="{{route('sw.deleteeSaleChannel',$sale_channel->id)}}"
                                        ...
                                        @else
                                            ...
                                            <a title="{{ trans('admin.disable')}}"
                                               href="{{route('sw.deleteSaleChannel',$sale_channel->id)}}"
            ...
            <div class="d-flex flex-stack flex-wrap pt-10">
                <div class="fs-6 fw-semibold text-gray-700">
                    Showing {{ $sale_channels->firstItem() ?? 0 }} to {{ $sale_channels->lastItem() ?? 0 }} of {{ $sale_channels->total() }} entries
                </div>
                <ul class="pagination">
                    {!! $sale_channels->appends($search_query)->render() !!}
                </ul>
            </div>
            <!--end::Pagination-->
        @else
