    ...
    <div class="card-header align-items-center py-5 gap-2 gap-md-5">
        ...
        <div class="card-title">
            <div class="d-flex align-items-center my-1">
                <i class="ki-outline ki-category fs-2 me-3"></i>
                <span class="fs-4 fw-semibold text-gray-900">{{ trans('sw.categories')}}</span>
            </div>
        </div>
        <!--end::Card title-->
        <div class="card-toolbar">
            <div class="d-flex align-items-center gap-2 gap-lg-3">
                <!--begin::Add Category-->
                @if(in_array('createCategory', (array)$swUser->permissions) || $swUser->is_super_user)
                    <a href="{{route('sw.createCategory')}}" class="btn btn-sm btn-flex btn-light-primary">
                ...
                <!--begin::Export-->
                @if((count(array_intersect(@(array)$swUser->permissions, ['exportCategoryPDF', 'exportCategoryExcel'])) > 0) || $swUser->is_super_user)
                    <div class="m-0">
                        ...
                        <div class="menu menu-sub menu-sub-dropdown w-200px" data-kt-menu="true">
                            @if(in_array('exportCategoryExcel', (array)$swUser->permissions) || $swUser->is_super_user)
                                <div class="menu-item px-3">
                                    <a href="{{route('sw.exportCategoryExcel')}}" class="menu-link px-3">
                                        <i class="ki-outline ki-file-down fs-6 me-2"></i>
                                        {{ trans('sw.excel_export')}}
                                    </a>
                                </div>
                            @endif
                            @if(in_array('exportCategoryPDF', (array)$swUser->permissions) || $swUser->is_super_user)
                                <div class="menu-item px-3">
                                    <a href="{{route('sw.exportCategoryPDF')}}" class="menu-link px-3">
                                        <i class="ki-outline ki-file-down fs-6 me-2"></i>
                                        {{ trans('sw.pdf_export')}}
                                    </a>
                                </div>
                ...
                <thead>
                    <tr class="text-gray-500 fw-bold fs-7 text-uppercase gs-0">
                        <th class="min-w-200px text-nowrap">
                            <i class="ki-outline ki-category fs-6 me-2"></i>{{ trans('sw.name')}}
                        </th>
                        ...
                        <tr>
                            ...
                            <td class="pe-0">
                                <div class="d-flex flex-wrap gap-2">
                                    <span class="badge badge-{{$category->is_subscription ? 'primary' : 'secondary'}} fs-7">
                                        <i class="ki-outline ki-credit-cart fs-6 me-1"></i>{{ trans('sw.subscriptions')}}
                                    </span>
                                    <span class="badge badge-{{$category->is_activity ? 'primary' : 'secondary'}} fs-7">
                                        <i class="ki-outline ki-list fs-6 me-1"></i>{{ trans('sw.activities')}}
                                    </span>
                                    <span class="badge badge-{{$category->is_pt_subscription ? 'primary' : 'secondary'}} fs-7">
                                        <i class="ki-outline ki-user fs-6 me-1"></i>{{ trans('sw.pt')}}
                                    </span>
                                    <span class="badge badge-{{$category->is_training ? 'primary' : 'secondary'}} fs-7">
                                        <i class="ki-outline ki-gym fs-6 me-1"></i>{{ trans('sw.training_plans')}}
                                    </span>
                                    <span class="badge badge-{{$category->is_store ? 'primary' : 'secondary'}} fs-7">
                            ...
                            <td class="text-end actions-column">
                                <div class="d-flex justify-content-end align-items-center gap-1 flex-wrap">
                                    @if(in_array('editCategory', (array)$swUser->permissions) || $swUser->is_super_user)
                                        <!--begin::Edit-->
                                        <a href="{{route('sw.editCategory',$category->id)}}"
                                    ...
                                    @if(in_array('deleteCategory', (array)$swUser->permissions) || $swUser->is_super_user)
                                        @if(request('trashed'))
                                            <!--begin::Enable-->
                                            <a title="{{ trans('admin.enable')}}"
                                               href="{{route('sw.deleteCategory',$category->id)}}"
