...
<div class="card card-flush">
    ...
    <div class="card-header align-items-center py-5 gap-2 gap-md-5">
        ...
        <div class="card-toolbar">
            <div class="d-flex align-items-center gap-2 gap-lg-3">
                <!--begin::Filter-->
                <button type="button" class="btn btn-sm btn-flex btn-light-primary" data-bs-toggle="collapse" data-bs-target="#kt_training_tasks_filter_collapse">
                    <i class="ki-outline ki-filter fs-6"></i>
                    {{ trans('sw.filter')}}
                </button>
                <!--end::Filter-->
                
                <!--begin::Add Training Task-->
                @if(in_array('createBlockMember', (array)$swUser->permissions) || $swUser->is_super_user)
                    <a href="{{route('sw.createTrainingTask')}}" class="btn btn-sm btn-flex btn-light-primary">
                        <i class="ki-outline ki-plus fs-6"></i>
                        {{ trans('admin.add')}}
                    </a>
                @endif
                <!--end::Add Training Task-->
                
                <!--begin::Export-->
                @if((count(array_intersect(@(array)$swUser->permissions, ['exportTrainingTaskPDF', 'exportTrainingTaskExcel'])) > 0) || $swUser->is_super_user)
                    <div class="m-0">
                        ...
                        <div class="menu menu-sub menu-sub-dropdown w-200px" data-kt-menu="true">
                            @if(in_array('exportTrainingTaskExcel', (array)$swUser->permissions) || $swUser->is_super_user)
                                <div class="menu-item px-3">
                                    <a href="{{route('sw.exportTrainingTaskExcel')}}" class="menu-link px-3">
                                        <i class="ki-outline ki-file-down fs-6 me-2"></i>
                                        {{ trans('sw.excel_export')}}
                                    </a>
                                </div>
                            @endif
                            @if(in_array('exportTrainingTaskPDF', (array)$swUser->permissions) || $swUser->is_super_user)
                                <div class="menu-item px-3">
                                    <a href="{{route('sw.exportTrainingTaskPDF')}}" class="menu-link px-3">
                                        <i class="ki-outline ki-file-down fs-6 me-2"></i>
                                        {{ trans('sw.pdf_export')}}
                                    </a>
                                </div>
    ...
    <div class="card-body pt-0">
        <!--begin::Filter-->
        <div class="collapse" id="kt_training_tasks_filter_collapse">
        ...
        @if(count($members) > 0)
            ...
            <div class="table-responsive">
                <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_training_tasks_table">
                <thead>
                ...
                <tbody class="fw-semibold text-gray-600">
                    @foreach($members as $key=> $member)
                        <tr>
                            <td class="pe-0">
                                <span class="fw-bold">{{ str_pad(@$member->member->code, 14, 0, STR_PAD_LEFT) }}</span>
                            </td>
                            <td>
                                <div class="d-flex align-items-center">
                                    <!--begin::Avatar-->
                                    <div class="symbol symbol-50px me-3">
                                        <img alt="avatar" class="rounded-circle" src="{{@$member->member->image}}" />
                                    </div>
                                    <!--end::Avatar-->
                                    <div>
                                        <!--begin::Title-->
                            ...
                            <td class="pe-0">
                                <button type="button" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm" 
                                        data-bs-toggle="modal" data-bs-target="#modal_task_note_{{$member->id}}" 
                                        title="{{ trans('sw.details')}}">
                                    <i class="ki-outline ki-information fs-2"></i>
                                </button>
                            </td>
                            <!-- notes -->
                            <div id="modal_task_note_{{$member->id}}" class="modal fade" role="dialog">
                                <div class="modal-dialog">
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <div class="row">
                                                <div class="col-md-6">
                                                    <h4><b>{{ trans('sw.height')}}:</b> {{$member->height}}</h4>
                                                    <p></p>

                                                    <p>
                                                        <b>{{ trans('sw.neck_circumference')}}:</b> {{@$member->neck_circumference ?? '-'}}
                                                    </p>
                                                    <p>
                                                        <b>{{ trans('sw.arm_circumference')}}:</b> {{@$member->arm_circumference ?? '-'}}
                                                    </p>
                                                    <p>
                                                        <b>{{ trans('sw.pelvic_circumference')}}:</b> {{@$member->pelvic_circumference ?? '-'}}
                                                    </p>

                                                </div>
                                                <div class="col-md-6">
                                                    <h4><b>{{ trans('sw.weight')}}:</b> {{$member->weight}}</h4>
                                                    <p></p>
                                                    <p>
                                                        <b>{{ trans('sw.chest_circumference')}}:</b> {{@$member->chest_circumference ?? '-'}}
                                                    </p>
                                                    <p>
                                                        <b>{{ trans('sw.abdominal_circumference')}}:</b> {{@$member->abdominal_circumference ?? '-'}}
                                                    </p>
                                                    <p>
                                                        <b>{{ trans('sw.thigh_circumference')}}:</b> {{@$member->thigh_circumference ?? '-'}}
                                                    </p>
                                                </div>
                                            </div>
                                            ...
                                            <p>{!! strip_tags($member->notes, '<p><br>') !!}</p>
                                        </div>
                                    </div>
                            ...
                            <td class="pe-0">
                                <div class="d-flex flex-column">
                                    <div class="text-muted fw-bold d-flex align-items-center">
                                        ...
                                        <span>{{ \Carbon\Carbon::parse($member->date)->format('Y-m-d') }}</span>
                                    </div>
                                </div>
                            </td>
                            <td class="text-end actions-column">
                                <div class="d-flex justify-content-end align-items-center gap-1 flex-wrap">
                                    @if(in_array('editTrainingTask', (array)$swUser->permissions) || $swUser->is_super_user)
                                        <!--begin::Edit-->
                                        <a href="{{route('sw.editTrainingTask',$member->id)}}"
                                    ...
                                    @if(in_array('deleteTrainingTask', (array)$swUser->permissions) || $swUser->is_super_user)
                                        @if(request('trashed'))
                                            <!--begin::Enable-->
                                            <a title="{{ trans('admin.enable')}}"
                                               href="{{route('sw.deleteTrainingTask',$member->id)}}"
