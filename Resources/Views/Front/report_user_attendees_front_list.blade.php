...
<div class="card card-flush">
    ...
    <div class="card-header align-items-center py-5 gap-2 gap-md-5">
        ...
        <div class="card-toolbar">
            <div class="d-flex flex-wrap align-items-center gap-2 gap-lg-3">
                <!--begin::Filter-->
                <button type="button" class="btn btn-sm btn-flex btn-light-primary" data-bs-toggle="collapse" data-bs-target="#kt_user_attendees_filter_collapse">
                ...
                <!--begin::Export-->
                @if((count(array_intersect(@(array)$swUser->permissions, ['exportUserAttendeesPDF', 'exportUserAttendeesExcel'])) > 0) || $swUser->is_super_user)
                    <div class="m-0">
                        ...
                        <div class="menu menu-sub menu-sub-dropdown w-200px" data-kt-menu="true">
                            @if(in_array('exportUserAttendeesExcel', (array)$swUser->permissions) || $swUser->is_super_user)
                                <div class="menu-item px-3">
                                    <a href="{{route('sw.exportUserAttendeesExcel', request()->query())}}" class="menu-link px-3">
                                        <i class="ki-outline ki-file-down fs-6 me-2"></i>
                                        {{ trans('sw.excel_export') }}
                                    </a>
                                </div>
                            @endif
                            @if(in_array('exportUserAttendeesPDF', (array)$swUser->permissions) || $swUser->is_super_user)
                                <div class="menu-item px-3">
                                    <a href="{{route('sw.exportUserAttendeesPDF', request()->query())}}" class="menu-link px-3">
                                        <i class="ki-outline ki-file-down fs-6 me-2"></i>
                                        {{ trans('sw.pdf_export') }}
                                    </a>
                                </div>
    ...
    <div class="card-body pt-0">
        <!--begin::Filter-->
        <div class="collapse" id="kt_user_attendees_filter_collapse">
        ...
        @if(count($logs) > 0)
            ...
            <div class="table-responsive">
                <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_user_attendees_table">
                    <thead>
                        <tr class="text-gray-500 fw-bold fs-7 text-uppercase gs-0">
                            ...
                            <th class="min-w-150px text-nowrap">
                                <i class="ki-outline ki-list fs-6 me-2"></i>{{ trans('sw.position')}}
                            </th>
                            <th class="min-w-100px text-nowrap">
                                ...
                                <td>
                                    <div class="d-flex align-items-center">
                                        <div class="symbol symbol-50px me-3">
                                            <img alt="avatar" class="rounded-circle" src="{{@$log->image}}">
                                        </div>
                                        <div>
                                ...
                                <td>
                                    <div class="d-flex flex-column">
                                        <div class="text-muted fw-bold d-flex align-items-center">
                                            ...
                                            <span>{{ @$log->created_at ? Carbon\Carbon::parse($log->created_at)->format('Y-m-d') : Carbon\Carbon::parse($log->updated_at)->format('Y-m-d')}}</span>
                                        </div>
                                        <div class="text-muted fs-7 d-flex align-items-center">
                                            <i class="ki-outline ki-time fs-6 text-muted me-2"></i>
                                            <span>{{ @$log->created_at ? Carbon\Carbon::parse($log->created_at)->format('h:i a') : Carbon\Carbon::parse($log->updated_at)->format('h:i a') }}</span>
                                        </div>
                                    </div>
                                </td>
