...
<body>
    ...
    <form method="post" action="{{ route('sw.store-invoice-mobile.submit') }}">
        {{ csrf_field() }}
        <input type="hidden" name="product_id" value="{{ $record->id }}">
        <input type="hidden" name="token" value="{{ request('token') }}">
        ...
        @if(!$currentUser)
            ...
            <div class="highlight-text">
                ...
                <div class="gender-row">
                    <input type="radio" name="gender" value="1" id="male_s" required>
                    <label for="male_s">{{ trans('front.male') }}</label>
                    <input type="radio" name="gender" value="2" id="female_s">
                    <label for="female_s">{{ trans('front.female') }}</label>
                </div>
