...
@section('breadcrumb')
    ...
    <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-1">
        ...
        <!--begin::Item-->
        <li class="breadcrumb-item text-muted">
            <a href="{{ route('sw.listStoreOrderVendor') }}" class="text-muted text-hover-primary">{{ trans('sw.purchase_invoices') }}</a>
        </li>
        <!--end::Item-->
        <!--begin::Item-->
        <li class="breadcrumb-item">
        ...
        <div class="mw-lg-950px mx-auto w-100">
                ...
                <!--begin::Wrapper-->
												<div class="d-flex flex-column gap-7 gap-md-10">
													...
													<div class="d-flex flex-column flex-sm-row gap-7 gap-md-10 fw-bold">
														...
														<div class="flex-root d-flex flex-column">
															<span class="text-muted">{{ trans('sw.date') }}</span>
														...
														<div class="flex-root d-flex flex-column">
															...
															<span class="fs-5">{{@$order->payment_type == 0 ? trans('sw.payment_cash') : (@$order->payment_type == 1 ? trans('sw.payment_online') : trans('sw.payment_bank_transfer'))}}</span>
														</div>
														<div class="flex-root d-flex flex-column">
															<span class="text-muted"><i class="ki-outline ki-bill fs-6 me-1"></i>{{ trans('sw.total_price') }}</span>
															<span class="fs-5">{{number_format($order['amount'], 2)}} {{@trans('sw.app_currency')}}</span>
														</div>
													</div>
													<!--end::Order details-->
													<!--begin::Billing & shipping-->
													@if(@$order->vendor_name || @$order->vendor_address || @$order->vendor_phone)
													<div class="d-flex flex-column flex-sm-row gap-7 gap-md-10 fw-bold">
														@if(@$order->vendor_name)
														<div class="flex-root d-flex flex-column" style="max-width: calc(25% - 20px);">
															<span class="text-muted"><i class="ki-outline ki-profile-user fs-6 me-1"></i>{{ trans('sw.vendor_name') }}</span>
															<span class="fs-5">{{@$order->vendor_name}}</span>
														</div>
														@endif
														@if(@$order->vendor_address)
														<div class="flex-root d-flex flex-column" style="max-width: calc(25% - 20px);">
															<span class="text-muted"><i class="ki-outline ki-geolocation fs-6 me-1"></i>{{ trans('sw.vendor_address') }}</span>
															<span class="fs-5">{{@$order->vendor_address}}</span>
														</div>
														@endif
														@if(@$order->vendor_phone)
														<div class="flex-root d-flex flex-column" style="max-width: calc(25% - 20px);">
															<span class="text-muted"><i class="ki-outline ki-phone fs-6 me-1"></i>{{ trans('sw.vendor_phone') }}</span>
															<span class="fs-5">{{@$order->vendor_phone}}</span>
														</div>
														@endif
                        ...
                        <!--begin::Table-->
                                ...
                                <thead>
																	<tr class="border-bottom fs-6 fw-bold text-muted">
																		<th class="min-w-175px pb-2">{{ trans('sw.product') }}</th>
																...
																<tbody class="fw-semibold text-gray-600">
																	@php
																		...
																		if(@$order->product && @$order->product->name) {
																			...
																			$productName = trans('sw.product') . ' #' . $order->id;
																		}
																		
																		if(@$order->notes) {
																			$productNote = $order->notes;
																		}
																	@endphp
																	<tr>
																		<td>
																			<div class="d-flex align-items-center">
																				...
																				<div class="ms-5">
																					...
																					@if($productNote)
																						<div class="fs-7 text-muted">{{ $productNote }}</div>
																					@endif
																				</div>
																				<!--end::Title-->
																			</div>
                                            </td>
																		<td class="text-end">{{$order->quantity ?? 1}}</td>
																		<td class="text-end">
                                    ...
                                    @else
																			...
																			@endif
																			{{@trans('sw.app_currency')}}
                                            </td>
																		<td class="text-end">{{number_format($order->amount, 2)}} {{@trans('sw.app_currency')}}</td>
                                        </tr>
																	@if(@$order['vat'])
																	<tr>
																		<td colspan="3" class="text-end">{{ trans('sw.invoice_total') }}</td>
																		<td class="text-end">{{number_format(($order['amount'] - @$order['vat'] ),2)}} {{@trans('sw.app_currency')}}</td>
																	</tr>
																	<tr>
																		<td colspan="3" class="text-end">{{ trans('sw.vat_total') }} ({{@$mainSettings->vat_details['vat_percentage'].'%'}})</td>
																		<td class="text-end">{{@number_format($order['vat'], 2)}} {{@trans('sw.app_currency')}}</td>
                                        </tr>
                                    @endif
																	<tr>
																		<td colspan="3" class="fs-3 text-gray-900 fw-bold text-end">{{ trans('sw.invoice_total') }}</td>
																		<td class="text-gray-900 fs-3 fw-bolder text-end">{{number_format($order['amount'], 2)}} {{@trans('sw.app_currency')}}</td>
                                        </tr>
                                </tbody>
                            </table>
                        </div>
            ...
            <div class="d-flex flex-stack flex-wrap mt-10 pt-8 {{ app()->getLocale() == 'ar' ? 'flex-row-reverse' : '' }}">
                ...
                <div class="my-1 {{ app()->getLocale() == 'ar' ? 'ms-5' : 'me-5' }}">
                    ...
                    <!-- end::Print-->
                    <!-- begin::POS View-->
                    <button type="button" class="btn btn-light-success btn-icon my-1" onclick="javascript:window.open('{{route('sw.showStoreOrderVendorPOS', @$order['id'])}}', 'POS','height=600,width=700');" title="POS {{ trans('sw.view') }}">
