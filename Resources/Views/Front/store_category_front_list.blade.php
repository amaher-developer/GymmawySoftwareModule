    ...
    <div class="card-header align-items-center py-5 gap-2 gap-md-5">
        ...
        <div class="card-title">
            <div class="d-flex align-items-center my-1">
                ...
                <span class="fs-4 fw-semibold text-gray-900">{{ trans('sw.store_categories')}}</span>
            </div>
        </div>
        <!--end::Card title-->
        <div class="card-toolbar">
            <div class="d-flex align-items-center gap-2 gap-lg-3">
                <!--begin::Add Category-->
                @if(in_array('createStoreCategory', (array)$swUser->permissions) || $swUser->is_super_user)
                    <a href="{{route('sw.createStoreCategory')}}" class="btn btn-sm btn-flex btn-light-primary">
                ...
                <!--begin::Export-->
                @if((count(array_intersect(@(array)$swUser->permissions, ['exportStoreCategoryPDF', 'exportStoreCategoryExcel'])) > 0) || $swUser->is_super_user)
                    <div class="m-0">
                        ...
                        <div class="menu menu-sub menu-sub-dropdown w-200px" data-kt-menu="true">
                            @if(in_array('exportStoreCategoryExcel', (array)$swUser->permissions) || $swUser->is_super_user)
                                <div class="menu-item px-3">
                                    <a href="{{route('sw.exportStoreCategoryExcel')}}" class="menu-link px-3">
                                        <i class="ki-outline ki-file-down fs-6 me-2"></i>
                                        {{ trans('sw.excel_export')}}
                                    </a>
                                </div>
                            @endif
                            @if(in_array('exportStoreCategoryPDF', (array)$swUser->permissions) || $swUser->is_super_user)
                                <div class="menu-item px-3">
                                    <a href="{{route('sw.exportStoreCategoryPDF')}}" class="menu-link px-3">
                                        <i class="ki-outline ki-file-down fs-6 me-2"></i>
                                        {{ trans('sw.pdf_export')}}
                                    </a>
                                </div>
    ...
    <div class="card-body pt-0">
                ...
                <thead>
                    <tr class="text-gray-500 fw-bold fs-7 text-uppercase gs-0">
                        <th class="min-w-200px text-nowrap">
                            <i class="ki-outline ki-shop fs-6 me-2"></i>{{ trans('sw.name')}}
                        </th>
                        ...
                        <tr>
                            ...
                            <td class="text-end">
                                <div class="d-flex justify-content-end align-items-center gap-1">
                                    @if(in_array('editStoreCategory', (array)$swUser->permissions) || $swUser->is_super_user)
                                        <!--begin::Edit-->
                                        <a href="{{route('sw.editStoreCategory',$category->id)}}"
                                    ...
                                    @if(in_array('deleteStoreCategory', (array)$swUser->permissions) || $swUser->is_super_user)
                                        <!--begin::Delete-->
                                        <a href="{{route('sw.deleteStoreCategory',$category->id)}}"
        ...
        @else
            <div class="text-center py-10">
                <i class="ki-outline ki-shop fs-5x text-gray-400"></i>
