...
@section('list_add_button')
    <a href="{{route('createGymUser')}}" class="btn btn-lg btn-success">{{trans('admin.add')}} GymUser</a>
     @if(request('trashed'))
            <a href="{{route('listGymUser')}}" class="btn btn-lg btn-info">{{trans('admin.enabled')}}</a>
        @else
            <a href="{{route('listGymUser')}}?trashed=1" class="btn btn-lg btn-danger">{{trans('admin.disabled')}}</a>
        @endif
...
@section('page_body')
    ...
    <table class="table table-striped table-bordered table-hover" >
        ...
        <tbody>
        @foreach($gymusers as $key=> $gymuser)
            <tr>
                <td> {{ $key+1 }}</td>
                <td> {{ $gymuser->id }}</td><td> {{ $gymuser->name }}</td>
                <td>
                    <a href="{{route('editGymUser',$gymuser->id)}}" class="btn btn-xs yellow">
                        <i class="fa fa-edit"></i>
                    </a>
                    @if(request('trashed'))
                        <a title="{{trans('admin.enable')}}" href="{{route('deleteGymUser',$gymuser->id)}}" class="confirm_delete btn btn-xs green">
                            <i class="fa fa-check-circle"></i>
                        </a>
                    @else
                        <a title="{{trans('admin.disable')}}" href="{{route('deleteGymUser',$gymuser->id)}}" class="confirm_delete btn btn-xs red">
     ...
     <div class="col-lg-5 col-md-5 col-md-offset-5">
                {!! $gymusers->appends($search_query)->render()  !!}
            </div>
        </div>
@endsection
