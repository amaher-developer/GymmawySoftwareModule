...
@section('list_add_button')
    <a href="{{route('createNonMember')}}" class="btn btn-lg btn-success">{{trans('admin.add')}} NonMember</a>
     @if(request('trashed'))
            <a href="{{route('listNonMember')}}" class="btn btn-lg btn-info">{{trans('admin.enabled')}}</a>
        @else
            <a href="{{route('listNonMember')}}?trashed=1" class="btn btn-lg btn-danger">{{trans('admin.disabled')}}</a>
        @endif
...
@section('page_body')
    ...
    <table class="table table-striped table-bordered table-hover" >
        ...
        <tbody>
        @foreach($nonmembers as $key=> $nonmember)
            <tr>
                <td> {{ $key+1 }}</td>
                <td> {{ $nonmember->id }}</td>
                <td>
                    <a href="{{route('editNonMember',$nonmember->id)}}" class="btn btn-xs yellow">
                        <i class="fa fa-edit"></i>
                    </a>
                    @if(request('trashed'))
                        <a title="{{trans('admin.enable')}}" href="{{route('deleteNonMember',$nonmember->id)}}" class="confirm_delete btn btn-xs green">
                            <i class="fa fa-check-circle"></i>
                        </a>
                    @else
                        <a title="{{trans('admin.disable')}}" href="{{route('deleteNonMember',$nonmember->id)}}" class="confirm_delete btn btn-xs red">
     ...
     <div class="col-lg-5 col-md-5 col-md-offset-5">
                {!! $nonmembers->appends($search_query)->render()  !!}
            </div>
        </div>
@endsection
