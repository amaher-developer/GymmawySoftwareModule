                            ...
                            <div class="row front-lists-v2 margin-bottom-15">
                                <div class="col-md-12">
                                    <ul class="list-unstyled">
                                       @php if(@count($member->member_subscription_info->activities) > 0){$currentLang = $lang ?? 'ar'; echo @implode(' ', array_map(function ($name) use ( $member, $currentLang){ if(@$name['activity']['id']) { static $i = 0;   return '<li><button class="btn btn-lg btn-'.(@$name['training_times'] > @$name['visits'] ? 'primary' : 'gray').' btn-xs rounded-3" id="activity_'.@$member->id.'_'.@$name['activity']['id'].'" onclick="non_membership_reservation('.@$member->id.', '.@$name['activity']['id'].')"  data-target="#modalReservation" data-toggle="modal" >'.$name['activity']['name_'.$currentLang]. ' ( '.@(int)$name['training_times'].' ) ' .'</button></li>';  $i++; } }, @$member->member_subscription_info->activities ?? [])); } @endphp
                                    </ul>
                                </div>
                            </div>
                            @else
