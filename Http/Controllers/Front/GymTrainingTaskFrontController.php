...
use Modules\Software\Repositories\GymTrainingTaskRepository;
...
class GymTrainingTaskFrontController extends GymGenericFrontController
{
    public $TrainingTaskRepository;
    ...
    public function index()
    {
        ...
        if(request('trashed'))
        {
            $members = GymTrainingTask::branch($this->user_sw->branch_setting_id, @$this->user_sw->tenant_id)->with('member')->onlyTrashed()->orderBy('id', 'DESC');
        }
        else
        {
            $members = GymTrainingTask::branch($this->user_sw->branch_setting_id, @$this->user_sw->tenant_id)->with('member')->orderBy('id', 'DESC');
        }
...
//            $query->orWhere('height', 'like', "%" . $search . "%");
        ...
        return view('software::Front.training_task_front_list', compact('members','title', 'total', 'search_query'));
    }

    function exportExcel(){
        $records = GymTrainingTask::branch($this->user_sw->branch_setting_id, @$this->user_sw->tenant_id)->with('member')->get();
...
//        })->download('xlsx');
    ...
    private function prepareForExport($data)
    {
        ...
        array_unshift($result, [trans('sw.training_tasks')]);
        return $result;
    }

    function exportPDF(){
        $records = $this->TrainingTaskRepository->get();
        $this->fileName = 'training_tasks-' . Carbon::now()->toDateTimeString();
        ...
        if ($this->lang == 'ar') {
            try {
                ...
                $notes = trans('sw.export_pdf_training_tasks');
    ...
    {
        $title = trans('sw.training_task_add');
        return view('software::Front.training_task_front_form', [
    ...
    public function store(GymTrainingTaskRequest $request)
    {
        ...
        $training_task_inputs = $this->prepare_inputs($request->except(['_token']));
        $training_task_inputs['user_id'] = $this->user_sw->id;
        $training_task_inputs['member_id'] = $member->id;
        $this->TrainingTaskRepository->create($training_task_inputs);


        $notes = str_replace(':name', $member->name, trans('sw.add_training_Task'));
    ...
    {
        $member = GymTrainingTask::branch($this->user_sw->branch_setting_id, @$this->user_sw->tenant_id)->with('member')->withTrashed()->find($id);
        $title = trans('sw.training_task_edit');
        return view('software::Front.training_task_front_form', ['member' => $member,'title'=>$title]);
    }
    ...
    public function update(GymTrainingTaskRequest $request, $id)
    {
        ...
        $member = $this->TrainingTaskRepository->withTrashed()->find($id);
        $training_task_inputs = $this->prepare_inputs($request->except(['_token']));
        $training_task_inputs['user_id'] = $this->user_sw->id;
        $training_task_inputs['member_id'] = $member_detail->id;

        $member->update($training_task_inputs);

        $notes = str_replace(':name', $member_detail->name, trans('sw.edit_training_task'));
...
//        $member->forceDelete();
        ...
        $notes = str_replace(':name', $member_detail->name, trans('sw.delete_training_task'));
    ...
    private function prepare_inputs($inputs)
    {
        ...
        $destinationPath = base_path(GymTrainingTask::$uploads_path);
        $ThumbnailsDestinationPath = base_path(GymTrainingTask::$thumbnails_uploads_path);
