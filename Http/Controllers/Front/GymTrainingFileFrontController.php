...
use Modules\Software\Exports\TrainingFileExport;
use Modules\Software\Http\Requests\GymTrainingFileRequest;
use Modules\Software\Models\GymMember;
use Modules\Software\Models\GymTrainingFile;
use Modules\Software\Repositories\GymTrainingFileRepository;
...
class GymTrainingFileFrontController extends GymGenericFrontController
{
    public $TrainingFileRepository;
    ...
    public function index()
    {
        ...
        $title = trans('sw.training_Files');
        $this->request_array = ['search', 'from', 'to'];
        $request_array = $this->request_array;
        foreach ($request_array as $item) $$item = request()->has($item) ? request()->$item : false;
        if(request('trashed'))
        {
            $members = GymTrainingFile::branch($this->user_sw->branch_setting_id, @$this->user_sw->tenant_id)->with('member')->onlyTrashed()->orderBy('id', 'DESC');
        }
        else
        {
            $members = GymTrainingFile::branch($this->user_sw->branch_setting_id, @$this->user_sw->tenant_id)->with('member')->orderBy('id', 'DESC');
        }
...
//            $query->orWhere('height', 'like', "%" . $search . "%");
        ...
        return view('software::Front.training_file_front_list', compact('members','title', 'total', 'search_query'));
    }

    function exportExcel(){
        $records = GymTrainingFile::branch($this->user_sw->branch_setting_id, @$this->user_sw->tenant_id)->with('member')->get();
...
//            $excel->setDescription(trans('sw.training_Files_data'));
//            $excel->sheet(trans('sw.training_Files_data'), function($sheet) use ($records) {
...
//        })->download('xlsx');
    ...
    private function prepareForExport($data)
    {
        ...
        array_unshift($result, [trans('sw.training_Files')]);
        return $result;
    }

    function exportPDF(){
        $records = $this->TrainingFileRepository->get();
        $this->fileName = 'training_files-' . Carbon::now()->toDateTimeString();
        ...
        if ($this->lang == 'ar') {
            try {
                ...
                $notes = trans('sw.export_pdf_training_files');
    ...
    {
        $title = trans('sw.training_file_add');
        return view('software::Front.training_file_front_form', [
            'member' => new GymTrainingFile(),'title'=>$title]);
    }

    public function store(GymTrainingFileRequest $request)
    {
        $member = GymMember::branch($this->user_sw->branch_setting_id, @$this->user_sw->tenant_id)->where('code', $request->member_id)->first();
        $training_file_inputs = $this->prepare_inputs($request->except(['_token']));
        $training_file_inputs['user_id'] = $this->user_sw->id;
        $training_file_inputs['member_id'] = $member->id;
        $this->TrainingFileRepository->create($training_file_inputs);


        $notes = str_replace(':name', $member->name, trans('sw.add_training_file'));
        $this->userLog($notes, TypeConstants::CreateTrainingFile);
        ...
        return redirect(route('sw.listTrainingFile'));
    }

    public function edit($id)
    {
        $member = GymTrainingFile::branch($this->user_sw->branch_setting_id, @$this->user_sw->tenant_id)->with('member')->withTrashed()->find($id);
        $title = trans('sw.training_file_edit');
        return view('software::Front.training_file_front_form', ['member' => $member,'title'=>$title]);
    }

    public function update(GymTrainingFileRequest $request, $id)
    {
        $member_detail = GymMember::branch($this->user_sw->branch_setting_id, @$this->user_sw->tenant_id)->where('code', $request->member_id)->first();
        $member = $this->TrainingFileRepository->withTrashed()->find($id);
        $training_file_inputs = $this->prepare_inputs($request->except(['_token']));
        $training_file_inputs['user_id'] = $this->user_sw->id;
        $training_file_inputs['member_id'] = $member_detail->id;

        $member->update($training_file_inputs);

        $notes = str_replace(':name', $member_detail->name, trans('sw.edit_training_file'));
        $this->userLog($notes, TypeConstants::EditTrainingFile);
...
//        $member->forceDelete();
        ...
        $notes = str_replace(':name', $member_detail->name, trans('sw.delete_training_file'));
        $this->userLog($notes, TypeConstants::DeleteTrainingFile);
    ...
    private function prepare_inputs($inputs)
    {
        ...
        $destinationPath = base_path(GymTrainingFile::$uploads_path);
        $ThumbnailsDestinationPath = base_path(GymTrainingFile::$thumbnails_uploads_path);
