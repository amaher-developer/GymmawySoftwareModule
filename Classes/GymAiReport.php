...
class GymAiReport
{
    ...
    private function lbl(): array
    {
        if ($this->lang === 'ar') {
            return [
                ...
                'training_recs'    => 'توصيات التدريب والتغذية',
                'loyalty_recs'     => 'توصيات برنامج الولاء',
        ...
        return [
            ...
            'no_data'          => 'No data',
            'training_recs'    => 'Training & Nutrition Recommendations',
            'loyalty_recs'     => 'Loyalty Program Recommendations',
    ...
    private function collectData(string $from, string $to): array
    {
        ...
        $currency = $this->lang === 'ar'
            ? ($this->settings->getAppConfig('currency_ar') ?: $this->settings->getAppConfig('currency_en', 'SAR'))
            : ($this->settings->getAppConfig('currency_en') ?: $this->settings->getAppConfig('currency_ar', 'SAR'));
        ...
        $totalAssessments  = GymTrainingAssessment::branch()->count();
        $activePlanMembers = GymTrainingMember::branch()->where('status', TypeConstants::Active)->count();

        // Loyalty Program Stats
        $hasLoyalty  = (bool)($this->settings->active_loyalty ?? false);
        $loyaltyData = ['enabled' => $hasLoyalty];

        if ($hasLoyalty) {
            $branchMemberIds = GymMember::branch()->pluck('id');
            $ltBase = fn() => LoyaltyTransaction::whereIn('member_id', $branchMemberIds);

            $membersWithPoints  = GymMember::branch()->where('loyalty_points_balance', '>', 0)->count();
            $totalActiveBalance = GymMember::branch()->sum('loyalty_points_balance');

            $pointsIssued   = (clone $ltBase())->where('type', LoyaltyTransaction::TYPE_EARN)
                ->whereDate('created_at', '>=', $from)->whereDate('created_at', '<=', $to)
                ->sum('points');
            $pointsRedeemed = (clone $ltBase())->where('type', LoyaltyTransaction::TYPE_REDEEM)
                ->whereDate('created_at', '>=', $from)->whereDate('created_at', '<=', $to)
                ->sum('points');
            $expiringPoints = (clone $ltBase())->where('type', LoyaltyTransaction::TYPE_EARN)
                ->where('is_expired', false)->whereNotNull('expires_at')
                ->whereBetween('expires_at', [now(), now()->addDays(30)])
                ->sum('points');

            $activeCampaign = LoyaltyCampaign::branch()->current()->first();

            $loyaltyData = [
                'enabled'                => true,
                'members_with_points'    => $membersWithPoints,
                'members_total_active'   => $totalActive,
                'engagement_rate'        => $totalActive > 0
                    ? round($membersWithPoints / $totalActive * 100, 1) . '%' : '0%',
                'total_active_balance'   => (int)$totalActiveBalance,
                'points_issued_period'   => (int)$pointsIssued,
                'points_redeemed_period' => abs((int)$pointsRedeemed),
                'redemption_rate'        => $pointsIssued > 0
                    ? round(abs($pointsRedeemed) / $pointsIssued * 100, 1) . '%' : '0%',
                'points_expiring_30d'    => (int)$expiringPoints,
                'active_campaign'        => $activeCampaign ? [
                    'name'       => $activeCampaign->name,
                    'multiplier' => (float)$activeCampaign->multiplier,
                ] : null,
            ];
        }
        ...
        return [
            ...
            'training'   => [
                ...
                'active_plan_members' => $activePlanMembers,
                'has_website'         => (bool) ($this->settings->active_website ?? false),
                'has_mobile_app'      => (bool) ($this->settings->active_mobile  ?? false),
            ],
            'loyalty'    => $loyaltyData,
            'attendance' => [
...
Return STRICT JSON using exactly this structure:
{
  ...
  "training_recommendations": {
    "overview": "",
    "assessment_tips": [],
    "training_and_diet_advice": [],
    "digital_engagement_tips": []
  },
...
- training_recommendations: advise the gym on growing assessment uptake, upselling training/diet plans, and digital channels (website & mobile app) — if has_website or has_mobile_app is false, include a strong recommendation to activate it.
- loyalty_recommendations: if loyalty.enabled is true, analyze engagement_rate (members_with_points / members_total_active), redemption_rate for program health, points_expiring_30d urgency, and active_campaign multiplier impact — give concrete tips to boost enrollment, drive redemption, and reduce churn through the loyalty program; if loyalty.enabled is false, include a strong recommendation to activate the loyalty program as a retention and revenue tool.
...
PROMPT;
    ...
    private function buildEmailHtml(array $r): string
    {
        ...
        if (!empty($r['training_recommendations']) && is_array($r['training_recommendations'])) {
            $tr        = $r['training_recommendations'];
            $trContent = '';

            if (!empty($tr['overview'])) {
                $trContent .= "<p style='margin:0 0 12px;color:#333;font-size:13px;line-height:1.85;border-{$accentSide}:3px solid #7d3c98;padding-{$accentSide}:12px;background:#f9f5fb;padding-top:8px;padding-bottom:8px'>"
                    . e($tr['overview']) . '</p>';
            }

            if (!empty($tr['assessment_tips'])) {
                $trContent .= "<p style='margin:0 0 6px;font-size:12px;font-weight:700;color:#7d3c98;text-transform:uppercase;letter-spacing:0.5px'>"
                    . ($isAr ? '📋 التقييمات' : '📋 Assessments') . '</p>'
                    . $ul((array)$tr['assessment_tips'], '#7d3c98')
                    . "<div style='margin-bottom:14px'></div>";
            }

            if (!empty($tr['training_and_diet_advice'])) {
                $trContent .= "<p style='margin:0 0 6px;font-size:12px;font-weight:700;color:#1a7a4a;text-transform:uppercase;letter-spacing:0.5px'>"
                    . ($isAr ? '🏋️ برامج التدريب والتغذية' : '🏋️ Training & Diet Programs') . '</p>'
                    . $ul((array)$tr['training_and_diet_advice'], '#1a7a4a')
                    . "<div style='margin-bottom:14px'></div>";
            }

            if (!empty($tr['digital_engagement_tips'])) {
                $trContent .= "<p style='margin:0 0 6px;font-size:12px;font-weight:700;color:#1a5276;text-transform:uppercase;letter-spacing:0.5px'>"
                    . ($isAr ? '📱 القنوات الرقمية' : '📱 Digital Channels') . '</p>'
                    . $ul((array)$tr['digital_engagement_tips'], '#1a5276');
            }

            if ($trContent) {
                $body .= $block('🧬', $lbl['training_recs'], $trContent, '#7d3c98');
            }
        }

        // Loyalty Program Recommendations
        $loyaltyColor = '#b7950b';  // gold
        if (!empty($r['loyalty_recommendations']) && is_array($r['loyalty_recommendations'])) {
            $lr        = $r['loyalty_recommendations'];
            $lrContent = '';

            if (!empty($lr['overview'])) {
                $lrContent .= "<p style='margin:0 0 12px;color:#333;font-size:13px;line-height:1.85;border-{$accentSide}:3px solid {$loyaltyColor};padding-{$accentSide}:12px;background:#fefce8;padding-top:8px;padding-bottom:8px'>"
                    . e($lr['overview']) . '</p>';
            }

            if (!empty($lr['engagement_tips'])) {
                $lrContent .= "<p style='margin:0 0 6px;font-size:12px;font-weight:700;color:{$loyaltyColor};text-transform:uppercase;letter-spacing:0.5px'>"
                    . ($isAr ? '🏅 تفعيل وإشراك الأعضاء' : '🏅 Member Engagement') . '</p>'
                    . $ul((array)$lr['engagement_tips'], $loyaltyColor)
                    . "<div style='margin-bottom:14px'></div>";
            }

            if (!empty($lr['campaign_ideas'])) {
                $lrContent .= "<p style='margin:0 0 6px;font-size:12px;font-weight:700;color:#d68910;text-transform:uppercase;letter-spacing:0.5px'>"
                    . ($isAr ? '🎯 أفكار الحملات' : '🎯 Campaign Ideas') . '</p>'
                    . $ul((array)$lr['campaign_ideas'], '#d68910')
                    . "<div style='margin-bottom:14px'></div>";
            }

            if (!empty($lr['retention_tips'])) {
                $lrContent .= "<p style='margin:0 0 6px;font-size:12px;font-weight:700;color:#1a7a4a;text-transform:uppercase;letter-spacing:0.5px'>"
                    . ($isAr ? '🔒 الاحتفاظ بالأعضاء' : '🔒 Retention Tips') . '</p>'
                    . $ul((array)$lr['retention_tips'], '#1a7a4a');
            }

            if ($lrContent) {
                $body .= $block('🏅', $lbl['loyalty_recs'], $lrContent, $loyaltyColor);
            }
        }
