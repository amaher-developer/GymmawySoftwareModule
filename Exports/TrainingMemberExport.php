...
class TrainingMemberExport implements FromCollection, WithHeadings, WithMapping, WithStyles, WithEvents, WithTitle
{
    use HasReportHeader;
    ...
    private function prepareForExcelValue($data)
    {
        foreach($this->keys as $key) {
            ...
            else if($key == 'plan_training')
                $arr[] = $data['training_plan_details'];
            else if($key == 'plan_diet')
                $arr[] = $data['diet_plan_details'];
            else
                $arr[] = $data[$key];
        }
        return $arr;
